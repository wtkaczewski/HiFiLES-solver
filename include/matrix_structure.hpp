...
private:
	...
	double *block;             /*!< \brief Internal hf_array to store a subblock of the matrix. */
	double *prod_block_vector; /*!< \brief Internal hf_array to store the product of a subblock with a vector. */
	double *prod_row_vector;   /*!< \brief Internal hf_array to store the product of a matrix-by-blocks "row" with a vector. */
	double *aux_vector;		   /*!< \brief Auxilar hf_array to store intermediate results. */
...
public:
	 ...
	 */
    void Initialize(int n_verts, int n_verts_global, int n_var, int n_eqns, hf_array<hf_array<int> > &v2e, hf_array<int> &v2n_e, hf_array<int> &e2v);
  ...
  /*!
	...
	void AddBlock(unsigned long block_i, unsigned long block_j, hf_array<double> val_block);
