...
void GeoPreprocess(struct solution* FlowSol, mesh &Mesh) {
  ...
  hf_array<int> f2c,f2loc_f,c2f,c2e,f2v,f2nv;
  hf_array<int> rot_tag,unmatched_inters;
  ...
  if(run_input.wall_model>0 or run_input.turb_model>0) {
    ...
    hf_array<int> n_seg_inters_hf_array(FlowSol->nproc);
    hf_array<int> n_tri_inters_hf_array(FlowSol->nproc);
    hf_array<int> n_quad_inters_hf_array(FlowSol->nproc);
    ...
    MPI_Allgather(&n_seg_noslip_inters, 1, MPI_INT, n_seg_inters_hf_array.get_ptr_cpu(), 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Allgather(&n_tri_noslip_inters, 1, MPI_INT, n_tri_inters_hf_array.get_ptr_cpu(), 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Allgather(&n_quad_noslip_inters, 1, MPI_INT, n_quad_inters_hf_array.get_ptr_cpu(), 1, MPI_INT, MPI_COMM_WORLD);
    ...
    // Allocate hf_arrays for coordinates of points on no-slip boundaries
    ...
    // Allocate global hf_arrays for coordinates of points on no-slip boundaries
    ...
    for(int i=0;i<FlowSol->n_ele_types;i++)
      FlowSol->mesh_eles(i)->calc_wall_distance_parallel(n_seg_inters_hf_array,n_tri_inters_hf_array,n_quad_inters_hf_array,FlowSol->loc_noslip_bdy_global,FlowSol->nproc);
              ...
              hf_array<int>& out_iv2ivg, int& out_n_cells, int& out_n_verts, int& out_n_verts_global, struct solution* FlowSol)
{
  ...
  // Call method to create hf_array iv2ivg and modify c2v using local vertex indices
  hf_array<int> iv2ivg;
  int n_verts;
...
void ReadBound(string& in_file_name, hf_array<int>& in_c2v, hf_array<int>& in_c2n_v, hf_array<int>& in_c2f, hf_array<int>& in_f2v, hf_array<int>& in_f2nv,
               ...
               hf_array<int>& in_ic2icg, hf_array<int>& in_icvsta, hf_array<int>&in_icvert, hf_array<int>& in_iv2ivg, int& in_n_cells, int& in_n_verts,
               struct solution* FlowSol)
{
  ...
  if (run_input.mesh_format==0) {
    hf_array<hf_array<int> > bccells;
    hf_array<hf_array<int> > bcfaces;
...
void create_boundpts(hf_array<hf_array<int> >& out_boundpts, hf_array<int>& in_bclist, hf_array<int>& out_bound_flag, hf_array<hf_array<int> >& in_bccells,
                     hf_array<hf_array<int> >& in_bcfaces, hf_array<int>& in_c2f, hf_array<int>& in_f2v, hf_array<int>& in_f2nv)
{
...
void read_boundary_gambit(string& in_file_name, int &in_n_cells, hf_array<int>& in_ic2icg, hf_array<int>& out_bctype, hf_array<int> &out_bclist,
                          hf_array<hf_array<int> >& out_bccells, hf_array<hf_array<int> >& out_bcfaces)
{
  ...
  hf_array<int> cell_list(in_n_cells);
...
void read_boundary_gmsh(string& in_file_name, int &in_n_cells, hf_array<int>& in_ic2icg, hf_array<int>& in_c2v, hf_array<int>& in_c2n_v, hf_array<int>& out_bctype,
                        hf_array<int> &out_bclist, hf_array<int> &out_bound_flag, hf_array<hf_array<int> >& out_boundpts, hf_array<int>& in_iv2ivg,
                        int in_n_verts, hf_array<int>& in_ctype, hf_array<int>& in_icvsta, hf_array<int>& in_icvert, struct solution* FlowSol)
{
  string str;
  ...
  hf_array<int> vlist_bound(9), vlist_cell(9);
  hf_array<int> vlist_local(9);
  ...
  // --- setup vertex->bcflag hf_array ---
  out_boundpts.setup(n_bcs);
  hf_array<set<int> > Bounds;
  Bounds.setup(n_bcs);
...
void create_iv2ivg(hf_array<int> &inout_iv2ivg, hf_array<int> &inout_c2v, int &out_n_verts, int in_n_cells)
{
  ...
  hf_array<int> vrtlist(in_n_cells*MAX_V_PER_C);
  hf_array<int> temp(in_n_cells*MAX_V_PER_C);
...
void repartition_mesh(int &out_n_cells, hf_array<int> &out_c2v, hf_array<int> &out_c2n_v, hf_array<int> &out_ctype, hf_array<int> &out_ic2icg, struct solution* FlowSol)
{
  ...
  hf_array<int> c2v_temp = out_c2v;
  hf_array<int> c2n_v_temp = out_c2n_v;
  hf_array<int> ctype_temp = out_ctype;
  hf_array<int> ic2icg_temp = out_ic2icg;
  ...
  //hf_array<int> part_hf_array(klocal);
  //for (i=0;i<klocal;i++)
  //{
  //  part_hf_array(i) = part[i];
  //}
  //part_hf_array.print_to_file(FlowSol->rank);
  //cout << "After print" << endl;

  // Now creating new c2v hf_array
  ...
  // Make exchange for hf_arrays c2v,c2n_v,ctype,ic2icg
  int cnt=0;
  for (int p=0;p<FlowSol->nproc;p++)
    {
...
void CompConnectivity(hf_array<int>& in_c2v, hf_array<int>& in_c2n_v, hf_array<int>& in_ctype, hf_array<int>& out_c2f, hf_array<int>& out_c2e,
                      hf_array<int>& out_f2c, hf_array<int>& out_f2loc_f, hf_array<int>& out_f2v, hf_array<int>& out_f2nv,
                      hf_array<int>& out_e2v, hf_array<int>& out_v2n_e, hf_array<hf_array<int> >& out_v2e,
                      hf_array<int>& out_rot_tag, hf_array<int>& out_unmatched_faces, int& out_n_unmatched_faces,
                      hf_array<int>& out_icvsta, hf_array<int>& out_icvert, int& out_n_faces, int& out_n_edges,
                      struct solution* FlowSol)
{
  ...
  //hf_array<int> num_v_per_c(5); // for 5 element types
  hf_array<int> vlist_loc(MAX_V_PER_F),vlist_loc2(MAX_V_PER_F),vlist_glob(MAX_V_PER_F),vlist_glob2(MAX_V_PER_F); // faces cannot have more than 4 vertices
  ...
  if (FlowSol->n_dims==3 || run_input.motion!=0)
  {
    ...
    vector<set<int> > v2e(n_verts);
      ...
      // Create hf_array ic2e
      hf_array<int> num_e_per_c(5);
...
void get_vlist_loc_edge(int& in_ctype, int& in_n_spts, int& in_edge, hf_array<int>& out_vlist_loc)
{
...
void get_vlist_loc_face(int& in_ctype, int& in_n_spts, int& in_face, hf_array<int>& out_vlist_loc, int& num_v_per_f)
{
