...
template <typename T>
void displayMatrix(hf_array<T> matrix) {
  int i,j;
...
void mesh::deform(struct solution* FlowSol) {
  hf_array<double> stiff_mat_ele;
  int failedIts = 0;

  hf_array<int> nodes; // NEW ADDITION 3/26/2014
...
bool mesh::set_2D_StiffMat_ele_tri(hf_array<double> &stiffMat_ele, int ele_id)
{
  int iPoint;
  int n_spts = c2n_v(ele_id);

  hf_array<double> pos_spts;
...
bool mesh::set_2D_StiffMat_ele_quad(hf_array<double> &stiffMat_ele,int ele_id) {
  FatalError("ERROR: Sorry, mesh motion on quads not yet implemented.  :( ");
}


// ---- **NEW** Added 3/26/14 ---- //
void mesh::set_stiffmat_ele_2d(hf_array<double> &stiffMat_ele, int ic, double scale)
{
  ...
  int nNodes = c2n_v(ic);
  hf_array<int> nodes(nNodes);
  hf_array<double> coords(nNodes,2);
...
// ---- **NEW** Added 3/26/14 ---- //
void mesh::set_stiffmat_ele_3d(hf_array<double> &stiffMat_ele, int ic, double scale)
{
...
// ---- **NEW** 3/26/14
void mesh::add_FEA_stiffMat(hf_array<double> &stiffMat_ele, hf_array<int> &PointCorners) {
 ...
 */
void mesh::add_StiffMat_EleTri(hf_array<double> StiffMatrix_Elem, int id_pt_0,
  ...
  hf_array<double> StiffMatrix_Node;
...
void mesh::add_StiffMat_EleQuad(hf_array<double> StiffMatrix_Elem, int id_pt_0,
...
void mesh::write_mesh_gmsh(double sim_time)
{
  ...
  /** ONLY FOR 2D CURRENTLY -- To fix, add hf_array<hf_array<int>> boundFaces to mesh class
...
void mesh::set_boundary_displacements(solution *FlowSol)
{
  ...
  hf_array<double> VarCoord(n_dims);
