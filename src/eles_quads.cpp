...
void eles_quads::set_vandermonde2D()
{
  ...
  // Store its inverse
  inv_vandermonde2D = inv_hf_array(vandermonde2D);
}

// Set the 2D inverse Vandermonde hf_array needed for shock capturing
void eles_quads::set_filter_hf_array()
{
...
void eles_quads::set_concentration_hf_array()
{
  ...
  concentration_hf_array.setup((order+1)*(order+1));
    ...
    for (int i=0;i<order+1;i++)
                for (int j=0;j<order+1;j++)
                        concentration_hf_array(j + i*(order+1)) = concentration_factor(j)*sqrt(1 - loc_1d_upts(i)*loc_1d_upts(i))*grad_vandermonde(i,j);
}
