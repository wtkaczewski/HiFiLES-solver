...
void eles::setup(int in_n_eles, int in_max_n_spts_per_ele)
{
  ...
  if (n_eles!=0)
  {
    ...
    // Allocate extra hf_arrays for LES models
    if(LES) {
    ...
    // Dummy hf_arrays to pass to GPU kernel wrapper
    else {
      disuf_upts.setup(1);
      Lu.setup(1);
      uu.setup(1);
    ...
    if (run_input.turb_model > 0) {
      ...
      zero_hf_array(wall_distance);
      zero_hf_array(wall_distance_mag);
      twall.setup(1);
    }
...
void eles::mv_all_cpu_gpu(void)
{
  ...
  if (n_eles!=0)
  {
    ...
    // Grid Velocity-related hf_arrays for moving meshes
    vel_spts.cp_cpu_gpu();
    ...
    if(run_input.ArtifOn){
        ...
        if(run_input.artif_type == 1)
        {
          concentration_hf_array.mv_cpu_gpu();
          sigma.mv_cpu_gpu();
        }
...
void eles::calc_sgs_terms(int in_disu_upts_from)
{
  if (n_eles!=0) {
    ...
    /*! If Similarity model, compute product terms and Leonard tensors */
      ...
      /*! third dimension of Lu, uu hf_arrays */
      if(n_dims==2)      dim3 = 3;
      else if(n_dims==3) dim3 = 6;
      
      /*! Calculate velocity and energy product hf_arrays uu, ue */
      for(i=0;i<n_upts_per_ele;i++) {
        for(j=0;j<n_eles;j++) {
          for(k=0;k<n_fields;k++) {
          ...
          /*! note that product hf_arrays are symmetric */
          if(n_dims==2) {
...
void eles::calc_sgsf_upts(hf_array<double>& temp_u, hf_array<double>& temp_grad_u, double& detjac, int ele, int upt, hf_array<double>& temp_sgsf)
{
  ...
  if(wall) {
    ...
    // Set hf_arrays for next timestep
...
void eles::calc_wall_distance_parallel(hf_array<int> n_seg_inters_hf_array, hf_array<int> n_tri_inters_hf_array, hf_array<int> n_quad_inters_hf_array, hf_array< hf_array<double> > loc_noslip_bdy_global, int nproc)
{
  if(n_eles!=0)
  {
    int i,j,k,m,n,p;
    ...
    for (i=0;i<n_eles;++i) {
      for (j=0;j<n_upts_per_ele;++j) {
        ...
        for (p=0;p<nproc;++p) {
          ...
          // line segment boundaries
          for (k=0;k<n_seg_inters_hf_array(p);++k) {
          ...
          // tri boundaries
          for (k=0;k<n_tri_inters_hf_array(p);++k) {
          ...
          // quad boundaries
          for (k=0;k<n_quad_inters_hf_array(p);++k) {
          ...
          tmp_sensor = 0;
            for(int i=0; i<in_order+1; i++)
            {
                ...
                for(int j=0; j<in_order+1; j++){
                    uE[j] = 0;
                    for(int k=0; k<in_order+1; k++)
                        uE[j] += modal_rho[k]*concentration_hf_array_ptr[j*(in_order+1) + k];
...
#ifdef _CPU
    hf_array_to_mklcsr(opp_0,opp_0_data,opp_0_cols,opp_0_b,opp_0_e);
#endif
    
#ifdef _GPU
    hf_array_to_ellpack(opp_0, opp_0_ell_data, opp_0_ell_indices, opp_0_nnz_per_row);
...
void eles::set_opp_1(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      hf_array_to_mklcsr(opp_1(i),opp_1_data(i),opp_1_cols(i),opp_1_b(i),opp_1_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      hf_array_to_ellpack(opp_1(i), opp_1_ell_data(i), opp_1_ell_indices(i), opp_1_nnz_per_row(i));
...
void eles::set_opp_2(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      hf_array_to_mklcsr(opp_2(i),opp_2_data(i),opp_2_cols(i),opp_2_b(i),opp_2_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      hf_array_to_ellpack(opp_2(i), opp_2_ell_data(i), opp_2_ell_indices(i), opp_2_nnz_per_row(i));
...
#ifdef _CPU
    hf_array_to_mklcsr(opp_3,opp_3_data,opp_3_cols,opp_3_b,opp_3_e);
#endif
    
#ifdef _GPU
    hf_array_to_ellpack(opp_3, opp_3_ell_data, opp_3_ell_indices, opp_3_nnz_per_row);
...
void eles::set_opp_4(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
    ...
    for (int i=0;i<n_dims;i++)
    {
      hf_array_to_mklcsr(opp_4(i),opp_4_data(i),opp_4_cols(i),opp_4_b(i),opp_4_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      hf_array_to_ellpack(opp_4(i), opp_4_ell_data(i), opp_4_ell_indices(i), opp_4_nnz_per_row(i));
...
void eles::set_opp_5(int in_sparse)
{
  ...
  else if(in_sparse==1)
  {
...
#ifdef _CPU
    for (int i=0;i<n_dims;i++) {
      hf_array_to_mklcsr(opp_5(i),opp_5_data(i),opp_5_cols(i),opp_5_b(i),opp_5_e(i));
    }
#endif
    ...
    for (int i=0;i<n_dims;i++) {
      hf_array_to_ellpack(opp_5(i), opp_5_ell_data(i), opp_5_ell_indices(i), opp_5_nnz_per_row(i));
...
#ifdef _CPU
    hf_array_to_mklcsr(opp_6,opp_6_data,opp_6_cols,opp_6_b,opp_6_e);
#endif
    
#ifdef _GPU
    hf_array_to_ellpack(opp_6, opp_6_ell_data, opp_6_ell_indices, opp_6_nnz_per_row);
...
void eles::calc_diagnostic_fields_ppts(int in_ele, hf_array<double>& in_disu_ppts, hf_array<double>& in_grad_disu_ppts, hf_array<double>& in_sensor_ppts, hf_array<double>& in_epsilon_ppts, hf_array<double>& out_diag_field_ppts, double& time)
{
  int i,j,k,m;
...
//          }
    ...
    // To avoid re-setting up ALL transform hf_arrays in the future (for dynamic grids)
    first_time = false;
  }
#endif
...
/*! Setup hf_arrays for storing grid velocity */
