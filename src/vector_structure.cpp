...
CSysVector::CSysVector(const unsigned long & size, const double* u_hf_array) {
...
void CSysVector::CopyToArray(double* u_hf_array) {
  
  for (unsigned long i = 0; i < nElm; i++)
    u_hf_array[i] = vec_val[i];
}
